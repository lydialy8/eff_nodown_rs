...
class Normalize(object):
    ...
    will normalize each channel of the input ``tf.*Tensor`` i.e.
    ``input[channel] = (input[channel] - mean[channel]) / std[channel]``
    .. note::
        This transform acts out of place, i.e., it does not mutates the input tensor.
    Args:
        tensor (tensor): input tf tensor data.
...
class GaussianBlur(object):
    ...
    def __call__(self, img):
        if random.random() < self.p:
            return F.gaussian_blur(img.numpy(), self.kernel_size)
        return img.numpy()
...
class RandomNoise(object):
    ...
    def __init__(self, mode="gaussian", percent=0.02, p = 0.1):
...
class RandomShift(object):
    ...
    def __init__(self, max_percent=0.4, p= 0.1):
...
class RandomRotation(object):
    ...
    def __init__(self, degrees, center=None, p= 0.1):
...
class Resize(object):
    ...
    def __call__(self, img):
        return F.resize(img.numpy(), self.size, self.interpolation)
